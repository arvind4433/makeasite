/* Shared auth page layout + reusable primitives */
import { Hexagon } from 'lucide-react';
import { Link } from 'react-router-dom';

const APP_NAME = import.meta.env.VITE_APP_NAME || 'WebDevPro';

/* Full-page wrapper */
export const AuthLayout = ({ children }) => (
    <div className="min-h-screen pt-20 flex items-center justify-center px-4 py-16 transition-colors duration-300"
        style={{ backgroundColor: 'var(--bg-base)' }}>

        {/* Decorative blobs */}
        <div className="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden>
            <div className="absolute -top-40 -right-40 w-96 h-96 rounded-full opacity-30 dark:opacity-60"
                style={{ background: 'radial-gradient(circle, rgba(220,38,38,0.12) 0%, transparent 70%)' }} />
            <div className="absolute -bottom-40 -left-40 w-96 h-96 rounded-full opacity-20 dark:opacity-50"
                style={{ background: 'radial-gradient(circle, rgba(220,38,38,0.08) 0%, transparent 70%)' }} />
        </div>

        <div className="relative z-10 w-full max-w-md">
            {children}
        </div>
    </div>
);

/* Logo + title block */
export const AuthHeader = ({ title, subtitle }) => (
    <div className="text-center mb-8">
        <Link to="/" className="inline-flex items-center gap-2 mb-6 group">
            <div className="p-2 rounded-xl text-white transition-shadow group-hover:shadow-[0_0_16px_rgba(220,38,38,0.5)]"
                style={{ background: 'linear-gradient(135deg,#dc2626,#b91c1c)' }}>
                <Hexagon className="h-5 w-5 fill-current" />
            </div>
            <span className="font-bold text-lg tracking-tight text-slate-900 dark:text-white">
                {APP_NAME.replace('Pro', '')}<span className="text-red-600 dark:text-red-500">Pro</span>
            </span>
        </Link>
        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-2">{title}</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{subtitle}</p>
    </div>
);

/* Card wrapper */
export const AuthCard = ({ children }) => (
    <div className="card px-7 py-8 sm:px-10 sm:py-10">{children}</div>
);

/* Input field */
export const AuthInput = ({ label, hint, ...props }) => (
    <div>
        <div className="flex items-center justify-between mb-1.5">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">{label}</label>
            {hint}
        </div>
        <input {...props} className="input-base" />
    </div>
);

/* Divider "Or continue with" */
export const AuthDivider = ({ text = 'Or continue with' }) => (
    <div className="relative my-6">
        <div className="absolute inset-0 flex items-center">
            <div className="w-full h-px" style={{ background: 'var(--border)' }} />
        </div>
        <div className="relative flex justify-center text-xs font-semibold">
            <span className="px-4 text-slate-500" style={{ backgroundColor: 'var(--bg-card)' }}>{text}</span>
        </div>
    </div>
);

/* Google SVG */
export const GoogleIcon = () => (
    <svg className="w-4.5 h-4.5 w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
    </svg>
);

/* GitHub SVG */
export const GitHubIcon = () => (
    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
        <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
    </svg>
);

/* Social buttons row */
export const SocialButtons = () => (
    <div className="grid grid-cols-2 gap-3">
        {[
            { icon: <GoogleIcon />, label: 'Google' },
            { icon: <GitHubIcon />, label: 'GitHub' },
        ].map(({ icon, label }) => (
            <button key={label}
                className="flex items-center justify-center w-full px-4 py-2.5 rounded-xl text-sm font-semibold transition-all hover:-translate-y-0.5"
                style={{ background: 'var(--bg-card-inner)', border: '1px solid var(--border-strong)', color: 'var(--text-secondary)' }}>
                {icon}{label}
            </button>
        ))}
    </div>
);
